...
class TargetCluster(TitleSlugDescriptionModel):
    ...
    telemetry_source = models.CharField(max_length=5, default="p",
        choices=TELEMETRY_SOURCE
    )
    config = JSONField(help_text="Configuration data stored as an encrypted\
        blob in the database", null=True
    )
    ...
    def add(cls, kubeconfig):
        ...
        for item in config_data.get("clusters", []):
            cluster, created = TargetCluster.objects.get_or_create(
                title=item.get("name"),
                api_endpoint=item.get("cluster", {}).get("server")
            )
            if created:
        ...
        for config_obj in split_kubeconfig(kubeconfig):
            cluster, created = TargetCluster.objects.get_or_create(
                title=config_obj.get("clusters", [])[0].get("name"),
                api_endpoint=config_obj.get("clusters", [])[0].get(
                    "cluster", {}).get("server")
            )
            ret_val.append(cluster)
            if created:
                ...
                cluster.save()
                log.info("Created new cluster record for {} @ {}".format(
                    cluster.title, cluster.api_endpoint)
                )
            else:
                log.warning("Cluster record for {} @ {} already existed - \
