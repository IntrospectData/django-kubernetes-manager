...
class KubernetesBase(TitleSlugDescriptionModel):
    ...
    id = models.UUIDField(default=uuid4, editable=False, primary_key=True,
        help_text="UUID Auto field.")
    cluster = models.ForeignKey('TargetCluster', on_delete=models.SET_NULL,
        null=True, help_text="ForeignKey to TargetCluster object."
    )
    config = JSONField(default=dict, null=True, blank=True,
        help_text="Pass in extra parameters here.")
    deployed = models.DateTimeField(null=True, blank=True,
        help_text="Time when object is applied to cluster.")
    removed = models.DateTimeField(null=True, blank=True,
        help_text="Time when object is removed from cluster.")
...
class KubernetesMetadataObjBase(KubernetesBase):
    ...
    labels = JSONField(default=dict,
        help_text="Dictionary store equivalent to Labels in Kubernetes API")
    annotations = JSONField(default=dict, null=True, blank=True,
        help_text="Dictionary store equivalent to Annotations in Kubernetes API")
...
class KubernetesNetworkingBase(KubernetesMetadataObjBase):
    ...
    api_version = models.CharField(max_length=16, default="v1",
        help_text="API version used to deploy child object.")
    kind = models.CharField(max_length=16,
        help_text="String representation of Kubernetes object kind")
    port = models.IntegerField(default=80,
        help_text="Port object will expose")
    namespace = models.ForeignKey(
        "KubernetesNamespace",
        on_delete=models.CASCADE,
        help_text="Live namespace the object is associated with.")
